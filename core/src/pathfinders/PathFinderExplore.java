 ...
 * enclosed by traps. For each trap section (a contiguous section of traps)
 * that it finds, it pushes a PathFinderEscape onto the stack that will get the
 * car adjacent to the trap section. It explores the room by hugging the left
 * wall, avoiding traps by hugging to the left of those too. Its job is done once
 * it gets back to where it started, upon which it gets popped off the stack and
...
public class PathFinderExplore extends PathFinderBasic {
    ...
    /** A single trap section is a HashMap<Coordinate, TrapTile>. We have multiple of these for each contiguous section of traps. */
     ...
     * there could be multiple points defining a path to follow. It also keeps
     * an eye on which traps it sees and records them.
    ...
    /**
     * This method looks to the left of the car, looking for traps.
     * When it sees a trap, it adds it to the currentTrapSection, which builds up
     * a contiguous section of traps. Once something breaks it up, we "commit" it
     * to trapSections, which is a list of all trap sections we've seen.
     */
    private void trackTraps() {
        ...
        // If the tile to our left is not a trap and we have a non-empty currentTrapSection, add it to the list of all trap sections.
    ...
    public boolean isDone() {
        ...
        if (done) {
            // A more complex implementation would rate each trap section and pick the section
